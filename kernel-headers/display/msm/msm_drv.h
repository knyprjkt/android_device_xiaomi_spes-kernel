...
#else
static __inline__ void __init msm_hdmi_register(void)
{
}
static __inline__ void __exit msm_hdmi_unregister(void)
{
}
#endif
...
int msm_edp_modeset_init(struct msm_edp *edp, struct drm_device *dev,
		struct drm_encoder *encoder);
#else
static __inline__ void __init msm_edp_register(void)
{
}
static __inline__ void __exit msm_edp_unregister(void)
{
}

static __inline__ int msm_edp_modeset_init(struct msm_edp *edp,
...
int msm_dsi_modeset_init(struct msm_dsi *msm_dsi, struct drm_device *dev,
			 struct drm_encoder *encoder);
#else
static __inline__ void __init msm_dsi_register(void)
{
}
static __inline__ void __exit msm_dsi_unregister(void)
{
}
static __inline__ int msm_dsi_modeset_init(struct msm_dsi *msm_dsi,
				       struct drm_device *dev,
				       struct drm_encoder *encoder)
{
	return -EINVAL;
...
#else
static __inline__ void __init msm_mdp_register(void)
{
}
static __inline__ void __exit msm_mdp_unregister(void)
{
}
#endif
...
#else
static __inline__ int msm_debugfs_late_init(struct drm_device *dev) { return 0; }
static __inline__ void msm_rd_dump_submit(struct msm_rd_state *rd, struct msm_gem_submit *submit,
		const char *fmt, ...) {}
static __inline__ void msm_rd_debugfs_cleanup(struct msm_drm_private *priv) {}
static __inline__ void msm_perf_debugfs_cleanup(struct msm_drm_private *priv) {}
#endif
...
struct clk *msm_clk_bulk_get_clock(struct clk_bulk_data *bulk, int count,
	const char *name);
void *msm_ioremap(struct platform_device *pdev, const char *name,
		const char *dbgname);
unsigned long msm_iomap_size(struct platform_device *pdev, const char *name);
void msm_iounmap(struct platform_device *dev, void *addr);
void msm_writel(u32 data, void *addr);
u32 msm_readl(const void *addr);

#define DBG(fmt, ...) DRM_DEBUG_DRIVER(fmt"\n", ##__VA_ARGS__)
#define VERB(fmt, ...) if (0) DRM_DEBUG_DRIVER(fmt"\n", ##__VA_ARGS__)

static __inline__ int align_pitch(int width, int bpp)
{
	int bytespp = (bpp + 7) / 8;
...
static __inline__ unsigned long timeout_to_jiffies(const ktime_t *timeout)
{
