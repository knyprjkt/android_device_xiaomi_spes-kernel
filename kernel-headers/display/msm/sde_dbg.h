...
#else
static __inline__ void sde_evtlog_log(struct sde_dbg_evtlog *evtlog, const char *name, int line,
		int flag, ...)
{
	return;
}
static __inline__ void sde_reglog_log(u8 blk_id, u32 val, u32 addr)
{
	return;
}
static __inline__ void sde_dbg_ctrl(const char *name, ...)
{
	return;
}
static __inline__ void sde_dbg_dump(enum sde_dbg_dump_context mode, const char *name, ...)
{
	return;
}
static __inline__ void sde_dbg_reg_register_dump_range(const char *base_name,
		const char *range_name, u32 offset_start, u32 offset_end,
		uint32_t xin_id)
{
	return;
}
static __inline__ void sde_dbg_set_sde_top_offset(u32 blk_off)
{
	return;
}
static __inline__ int sde_dbg_reg_register_base(const char *name, void *base,
		size_t max_offset)
{
	return 0;
}
static __inline__ int sde_dbg_init(struct device *dev) {
	return 0;
}
static __inline__ void sde_dbg_destroy(void) {
	return;
}
static __inline__ int sde_dbg_debugfs_register(struct device *dev) {
	return 0;
}
static __inline__ void sde_dbg_init_dbg_buses(u32 hwversion) {
	return;
}
