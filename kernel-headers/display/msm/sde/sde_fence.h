...
#else
static __inline__ void *sde_sync_get(uint64_t fd)
{
	return NULL;
}

static __inline__ void sde_sync_put(void *fence)
{
}

static __inline__ signed long sde_sync_wait(void *fence, long timeout_ms)
{
	return 0;
}

static __inline__ uint32_t sde_sync_get_name_prefix(void *fence)
{
	return 0x0;
}

static __inline__ struct sde_fence_context *sde_fence_init(const char *name,
		uint32_t drm_id)
{
	/* do nothing */
	return NULL;
...
static __inline__ void sde_fence_deinit(struct sde_fence_context *fence)
{
	/* do nothing */
}

static __inline__ int sde_fence_get(struct sde_fence_context *fence, uint64_t *val)
{
	return -EINVAL;
}

static __inline__ void sde_fence_signal(struct sde_fence_context *fence,
						ktime_t ts, bool reset_timeline)
{
	/* do nothing */
}

static __inline__ void sde_fence_prepare(struct sde_fence_context *fence)
{
	/* do nothing */
}

static __inline__ int sde_fence_create(struct sde_fence_context *fence,
						uint64_t *val, uint32_t offset)
{
	return 0;
}

static __inline__ void sde_fence_timeline_status(struct sde_fence_context *ctx,
					struct drm_mode_object *drm_obj);
{
	/* do nothing */
}
