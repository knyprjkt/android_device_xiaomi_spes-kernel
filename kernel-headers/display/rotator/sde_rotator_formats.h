...
static __inline__ bool sde_mdp_is_tilea4x_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->frame_format == SDE_MDP_FMT_TILE_A4X);
}

static __inline__ bool sde_mdp_is_tilea5x_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->frame_format == SDE_MDP_FMT_TILE_A5X);
}

static __inline__ bool sde_mdp_is_ubwc_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->is_ubwc == SDE_MDP_COMPRESS_UBWC);
}

static __inline__ bool sde_mdp_is_linear_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->frame_format == SDE_MDP_FMT_LINEAR);
}

static __inline__ bool sde_mdp_is_nv12_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->fetch_planes == SDE_MDP_PLANE_PSEUDO_PLANAR) &&
			(fmt->chroma_sample == SDE_MDP_CHROMA_420);
}

static __inline__ bool sde_mdp_is_nv12_8b_format(struct sde_mdp_format_params *fmt)
{
	return fmt && sde_mdp_is_nv12_format(fmt) &&
			(fmt->pixel_mode == SDE_MDP_PIXEL_NORMAL);
}

static __inline__ bool sde_mdp_is_nv12_10b_format(struct sde_mdp_format_params *fmt)
{
	return fmt && sde_mdp_is_nv12_format(fmt) &&
			(fmt->pixel_mode == SDE_MDP_PIXEL_10BIT);
}

static __inline__ bool sde_mdp_is_tp10_format(struct sde_mdp_format_params *fmt)
{
	return fmt && sde_mdp_is_nv12_10b_format(fmt) &&
			fmt->unpack_tight;
}

static __inline__ bool sde_mdp_is_p010_format(struct sde_mdp_format_params *fmt)
{
	return fmt && sde_mdp_is_nv12_10b_format(fmt) &&
			!fmt->unpack_tight;
}

static __inline__ bool sde_mdp_is_yuv_format(struct sde_mdp_format_params *fmt)
{
	return fmt && fmt->is_yuv;
}

static __inline__ bool sde_mdp_is_rgb_format(struct sde_mdp_format_params *fmt)
{
	return !sde_mdp_is_yuv_format(fmt);
}

static __inline__ bool sde_mdp_is_private_format(struct sde_mdp_format_params *fmt)
{
	return fmt && (fmt->flag & SDE_MDP_FORMAT_FLAG_PRIVATE);
}

static __inline__ int sde_mdp_format_blk_size(struct sde_mdp_format_params *fmt)
{
