...
static __inline__ size_t fp_frac_base(void)
{
	return GENMASK(FP_FRACTIONAL_BITS - 1, 0);
}

static __inline__ size_t fp_frac(fp_t a)
{
	return a & GENMASK(FP_FRACTIONAL_BITS - 1, 0);
}

static __inline__ size_t fp_int(fp_t a)
{
	return a >> FP_FRACTIONAL_BITS;
}

static __inline__ size_t fp_round(fp_t a)
{
...
static __inline__ fp_t fp_mult(fp_t a, fp_t b)
{
	return (a * b) >> FP_FRACTIONAL_BITS;
}


static __inline__ fp_t fp_div(fp_t a, fp_t b)
{
